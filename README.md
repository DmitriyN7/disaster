...
- engineered text meta-features include word count, unique word count, stop-word count, character count, punctuation count, hashtag count, and mention count.
  ...
  **Main model:** a fast TF-IDF word/character n-gram logistic-regression pipeline trains on cleaned keyword, location, and tweet text, tunes the decision threshold on validation F1, and refuses to write a single-class submission.
...
The default submission path uses a TF-IDF logistic-regression model configured in `src/train.py`. The optional transformer training parameters are defined in `src/config.py`:
...
- `artifacts/best_model/training_metadata.json` with the validation F1, tuned threshold, seed, and default model name.
- `submission.csv` with Kaggle test-set `id,target` predictions generated by the calibrated TF-IDF model.
